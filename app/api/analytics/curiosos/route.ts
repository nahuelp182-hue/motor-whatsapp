...
export async function GET() {
  ...
  const rows = await prisma.$queryRaw<Row[]>`
      ...
      SELECT vis.id, vis.ft_channel, vis.ft_at, vis.purchased_at,
        (SELECT count(*) FROM "Visit" t
           WHERE t.visitor_id = vis.id AND t.engaged
             AND t.started_at::date <> vis.ft_at::date) AS ret_engaged,
    ...
    SELECT ft_channel,
      count(*) AS visitors,
      round(avg(first_dur) / 1000.0, 1) AS avg_first_dwell_s,
      round(avg(first_scroll), 1) AS avg_first_scroll,
      round(100.0 * avg((first_engaged)::int), 1) AS pct_engaged_first,
      round(100.0 * avg((ret_engaged > 0)::int), 1) AS return_rate,
      round(100.0 * avg((purchased_at IS NOT NULL)::int), 1) AS purchase_rate,
      round(100.0 * avg((purchased_at IS NOT NULL AND purchased_at <= ft_at + interval '30 days')::int), 1) AS purchase_30d,
      round(100.0 * avg((purchased_at IS NOT NULL AND purchased_at <= ft_at + interval '60 days')::int), 1) AS purchase_60d,
      round(100.0 * avg((purchased_at IS NOT NULL AND purchased_at <= ft_at + interval '90 days')::int), 1) AS purchase_90d,
      round(percentile_cont(0.5) WITHIN GROUP (
        ORDER BY EXTRACT(EPOCH FROM (purchased_at - ft_at)) / 86400.0)
        FILTER (WHERE purchased_at IS NOT NULL)::numeric, 1) AS median_days_to_purchase
    FROM v
    GROUP BY ft_channel
