...
import { ultimaDerivacion } from '@/lib/diag'
  ...
  const { rows } = await pool.query<{ sender: string }>(
    `SELECT DISTINCT sender FROM ig_diag
      WHERE canal = 'wa' AND kind = 'handoff_activo'
        AND ts > now() - interval '${HANDOFF_HORAS} hours'`,
  )

  let avisados = 0
  for (const { sender } of rows) {
    const desde = await ultimaDerivacion(sender, HANDOFF_HORAS)
    ...
    const { permitido } = await consumirLimite(`watchdog:zona_ciega:${sender}`, 1, REALERTA_H * 3600)
