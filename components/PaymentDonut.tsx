...
export function PaymentDonut({ data }: { data: PaymentItem[] }) {
  if (!data.length) return (
    <div className="flex items-center justify-center h-full text-[var(--pnl-text-3)] text-xs">Sin datos</div>
  )
          ...
          <div key={d.label}>
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-[var(--pnl-text-2)] truncate max-w-[140px]">{d.label}</span>
              </div>
